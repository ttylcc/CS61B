/**
 * Created by DELL14R1528 on 2016/3/25.
 */
/* Date.java */
...
class Date {
  ...
  /* Put your private data fields here. */
    ...
    public Date(String s) {
        ...
        String m = s.substring(0,loc[0]);
        String d = s.substring(loc[0]+1,loc[1]);
        String y = s.substring(loc[1]+1,s.length());

        int mm = Integer.valueOf(m).intValue();
        int dd = Integer.valueOf(d).intValue();
...
//        return 0;                           // replace this line with your solution
    ...
    public static boolean isValidDate(int month, int day, int year) {
        return !((year<1) || (month>12 || month<1) || (day>daysInMonth(month, year)));
...
//        return res;                           // replace this line with your solution
    ...
    public int difference(Date d) {
        int res = 0;
        if (this.year == d.year) res = this.dayInYear()-d.dayInYear();
        else {
            for (int i=this.year>d.year?d.year:this.year; i<(this.year>d.year?this.year:d.year); i++){
                if (isLeapYear(i)){
                    res+=366;
                }
                else res+=365;
            ...
            if (this.year>d.year){
                res =res-d.dayInYear()+this.dayInYear();
            }
            else {
                res = -(res-this.dayInYear()+d.dayInYear());
            }
        }
